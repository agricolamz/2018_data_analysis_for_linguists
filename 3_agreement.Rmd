...
* каждый столбец — это суждения одного оценщика (raters)
* каждая строчка — это то, что оценивают (subjects)
...
### 4. Каппа Коэна
Каппа Коэна мера согласованности между двумя категориальными переменными $X$ и $Y$. Обычно говорят о двух оценщиках, которые распеделяют $n$ наблюдений по $s$ категориям.
...
p_o <- (47+46)/(47+46+4+9)
p_o
p_b <- ((47+9)/(47+46+4+9)) * ((47+4)/(47+46+4+9))
p_b
p_r <- ((46+4)/(47+46+4+9)) * ((46+9)/(47+46+4+9))
p_r
p_e <- p_b + p_r
p_e
...
Каппа Коэна — не единственная мера согласия между двумя оценщиками.

<div class="parallax"></div>

### 5. Каппа Фляйса
Обощением каппы Коэна для оценщиков больше 2 является каппа Фляйса. $k$ оценщиков распеделяют $n$ наблюдений по $s$ категориям.
...
```{r}
icc(marginal_verbs_short[-c(1:3)])
```
